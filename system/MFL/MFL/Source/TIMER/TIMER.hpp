...
class TIMER {
public:
    static auto get_instance(TIMER_Base Base) -> Result<TIMER, TIMER_Error_Type>;
    ...
    auto get_capture_compare(Timer_Channel channel) -> uint32_t;
    ...
    [[nodiscard]] inline auto reg_address(TIMER_Regs reg) const -> volatile uint32_t* {
        ...
        return reinterpret_cast<volatile uint32_t*>(TIMER_baseAddress[idx] + static_cast<uint32_t>(reg));
    }
...
private:
    ...
    [[nodiscard]] inline auto is_advanced_timer() const -> bool {
        return (base_ == TIMER_Base::TIMER0_BASE || base_ == TIMER_Base::TIMER7_BASE);
    }

    [[nodiscard]] inline auto is_general_purpose_timer() const -> bool {
    ...
    [[nodiscard]] inline auto is_basic_timer() const -> bool {
