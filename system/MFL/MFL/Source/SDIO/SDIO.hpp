...
public:
    ...
    // Get clock divider
    auto get_clock_divider() -> uint32_t;
    ...
    auto get_command_index() -> uint8_t;
    auto get_response(Response_Type type) -> uint32_t;
    ...
    // Data read/write
    auto read_fifo_word() -> uint32_t;
    void write_fifo_word(uint32_t data);

    // Count
    auto get_data_count() -> uint32_t;
    auto get_fifo_count() -> uint32_t;
    ...
    auto any_flag_set(const Status_Flags* flags, size_t count) -> bool;
    ...
    // Check SCR flags
    auto check_scr_flags() -> bool;

    // Wait for valid flag
    auto wait_cmd_flags() -> bool;

    // Interrupt flags
    auto get_interrupt_flag(Interrupt_Flags int_flag) -> bool;
    ...
    inline auto calculate_clock_divider(uint32_t requested_clock) -> uint16_t {
        ...
        requested_clock = custom_min(requested_clock, (sdio_pclk * 8 / 3));
        requested_clock = custom_min(requested_clock, Default_Desired_Clock);

        return static_cast<uint16_t>((sdio_pclk + requested_clock - 1U) / requested_clock - 2U);
    }

    static inline constexpr uintptr_t SDIO_baseAddress = 0x4001'8000U;

    [[nodiscard]] inline auto reg_address(SDIO_Regs reg) const -> volatile uint32_t* {
...
private:
    ...
    auto operator=(const SDIO&) -> SDIO& = delete;
