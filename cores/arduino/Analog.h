...
inline constexpr adc::ADC_Sample_Time SampleTime = adc::ADC_Sample_Time::SAMPLETIME_13_5_CYCLES;
inline constexpr adc::ADC_Sample_Time InternalSampleTime = adc::ADC_Sample_Time::SAMPLETIME_239_5_CYCLES;
...
inline const auto getAdcChannel(pin_size_t pin) -> adc::ADC_Channel {
    ...
    auto packedPinOps = getPackedPinOps(ADC_PinOps, pin);
 ...
 */
inline constexpr auto getAdcInternalChannel(pin_size_t pin) -> adc::ADC_Channel {
 ...
 *         pin is out of range
 */
inline auto getAdcChannelFromPin(pin_size_t pin) -> adc::ADC_Channel {
 ...
 */
inline auto getAdcSampleTime(pin_size_t pin) -> adc::ADC_Sample_Time {
 ...
 */
inline constexpr auto getAdcResolution(uint32_t resolution) -> adc::ADC_Resolution {
    switch (resolution) {
 ...
 */
inline auto getPwmResolution(uint32_t resolution) -> CCFormat {
    constexpr CCFormat formats[] = {
 ...
 */
inline auto mapResolution(uint32_t value, uint32_t from, uint32_t to) -> uint32_t {
    if (from == to) {
        return value;
    }
