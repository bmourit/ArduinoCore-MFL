...
public:
    ...
    auto attach(int pin) -> uint8_t;                    // attach the given pin to the next free channel, sets pinMode, returns channel number or 0 if failure
    auto attach(int pin, int min, int max) -> uint8_t;  // as above but also sets min and max values for writes.
    void detach();
    void write(int value);                      // if value is < 200 its treated as an angle, otherwise as pulse width in microseconds
    void writeMicroseconds(int value);          // Write pulse width in microseconds
    auto read() -> int;                                 // returns current pulse width as an angle between 0 and 180 degrees
    auto readMicroseconds() -> int;                     // returns current pulse width in microseconds for this servo (was read_us() in first release)
    auto attached() -> bool;                            // return true if this servo is attached, otherwise false
